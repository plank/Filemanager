...
class Filemanager {
	...
	public function __construct($config) {
		...
		$this->properties = array(
			...
			'Width'=>null,
			'Size'=>null,
			'Session'=>basename(dirname(dirname(dirname(dirname(dirname(__FILE__)))))).'/index.php'
		);
		$this->setParams();
	}
	...
	public function addfolder() {
		if(is_dir($_SERVER['DOCUMENT_ROOT'] . $this->get['path'] . $this->get['name'])) {
			$realPath = $_SERVER['DOCUMENT_ROOT'] . $this->get['path'] . $this->get['name'];
			// $this->error(sprintf($this->lang('DIRECTORY_ALREADY_EXISTS'),$this->get['name']));
			$this->error(sprintf($this->lang('DIRECTORY_ALREADY_EXISTS'),$realPath));
		}
		if(!mkdir($_SERVER['DOCUMENT_ROOT'] . $this->get['path'] . $this->get['name'],0755)) {
			$pathName = $_SERVER['DOCUMENT_ROOT'] . $this->get['path'] . $this->get['name'];
			// $this->error(sprintf($this->lang('UNABLE_TO_CREATE_DIRECTORY'),$this->get['name']));
			$this->error(sprintf($this->lang('UNABLE_TO_CREATE_DIRECTORY'),$pathName));
		}
		$array = array(
