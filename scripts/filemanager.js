...
var bindToolbar = function(data){
	...
	$('#fileinfo').find('button#select').click(function(){
		if(current_url == productionURL) {
			data.Path = data.Path.replace('webroot','');
			data.Path = 'http://'+productionURL+data.Path;
		}
		if(current_url == developmentURL) {
			data.Path = data.Path.replace('/webroot','');
			data.Path = 'http://'+developmentURL+data.Path;
		}
		// data.Path = data.Path.replace('/webroot/','');
		// 	data.Path = data.Path.replace('/centah','');
		// 	data.Path = data.Path.replace('/uploads','uploads');
		selectItem(data);
	});
...
var selectItem = function(data){
	// console.log(data); // uploads/cliff.jpg
